...
function SetInpPortDims(block, idx, di)
  ...
  block.OutputPort(1).Dimensions  = 8*(block.NumInputPorts+1);
...
function DoPostPropSetup(block)
  ...
  block.Dwork(1).Dimensions      = block.NumInputPorts +1;
...
function Start(block)
  Header=typecast(225563544,'double');
    data=Header;
    for i=1:block.NumInputPorts
        data=[data,block.InputPort(i).Data];
    end
...
function Outputs(block)
   ...
   block.OutputPort(1).Data =typecast(block.Dwork(1).Data,'uint8');
